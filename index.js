...
// The inner function has access to all the variables and functions defined in the outer function, 
// including its parameters. This means that even after the outer function has finished executing, the inner 
// function still has access to the variables defined in the outer function scope. In this code, the nested 
// function can access the "internal" variable because it is defined inside the "myFunction" scope.
...
function summation(givenNumber) {
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(zooAnimals){
    // initialize a new array `displayNames`:
    let displayNames = [];

    // populate `displayNames` by pushing a string containing animal_name and scientific_name to the end of the array:
  ...
  */
  function lowerCaseNames(zooAnimals) {
    // use method `.toLowerCase()` to convert each animal_name to lower case:
    let res = zooAnimals.map(animal => animal.animal_name.toLowerCase())
    return res;
}
  ...
  */
  ...
  function USApop(zooAnimals) {
    // here, the callback fn is `(acc, curr) => acc + curr.population`. It's created as an arrow function.
    // we use an initial value of 0:
    return zooAnimals.reduce((acc, curr) => acc + curr.population, 0);
}
